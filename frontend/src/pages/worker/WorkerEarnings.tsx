  ...
  return (
    <div className="app-container with-bottom-nav">
      ...
      <div className="app-header" style={{ padding: '16px 20px', backgroundColor: 'var(--primary)', color: 'white' }}>
        <h1 style={{ fontSize: '20px', fontWeight: 600, margin: 0 }}>Earnings & Payouts</h1>
      </div>
      ...
      <main style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px' }}>
          <div style={{ flex: 1, backgroundColor: 'var(--bg-card)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: 500 }}>Platform Fees</span>
            </div>
          ...
          <div style={{ flex: 1, backgroundColor: 'var(--bg-card)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-muted)' }}>
              <ShieldCheck size={16} />
              <span style={{ fontSize: '12px', fontWeight: 500 }}>Welfare Fund</span>
            </div>
            <span style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-main)' }}>
        ...
        {/* Transactions List */}
        <div>
          <h3 style={{ fontSize: '16px', fontWeight: 600, margin: '0 0 16px 0', color: 'var(--text-main)' }}>Recent Transactions</h3>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '30px 20px', backgroundColor: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)' }}>
              <p className="text-muted" style={{ margin: 0, fontSize: '14px' }}>No transactions yet.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {transactions.map((t: any) => (
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'var(--bg-card)', padding: '16px', borderRadius: '12px', border: '1px solid var(--border)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      <h4 style={{ margin: '0 0 4px 0', fontSize: '15px', fontWeight: 600 }}>Job Payout</h4>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <h4 style={{ margin: '0 0 4px 0', fontSize: '15px', fontWeight: 700, color: '#16A34A' }}>+ ₹{t.net_amount}</h4>
                  </div>
                </div>
              ))}
            </div>
      ...
      <nav className="bottom-nav">
        ...
        <button className="nav-item active" onClick={() => navigate('/worker/earnings')}>
          <IndianRupee size={24} />
          <span>Earnings</span>
        </button>
